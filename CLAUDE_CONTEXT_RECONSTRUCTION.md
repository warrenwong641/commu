...
Force-minimal projection may override stale-turn relevance or constraint
protection only when needed to meet the configured token budget. Every such
override is recorded in `protected_budget_evicted_turn_ids`. The configured
minimum recent window is never evicted; if that window plus projection overhead
cannot fit, `budget_satisfied` is false and the experiment runner records a
non-analyzable `COMPRESSION_BUDGET_EXCEEDED` row without invoking the model.
