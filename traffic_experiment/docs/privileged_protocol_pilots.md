...
is admitted only after a bounded check verifies that the exact Caddy-owned
TCP/UDP listeners bind the configured experiment-side veth address rather than
a wildcard or public interface, then completes TLS 1.3/HTTP/1.1 and
HTTP/3/QUIC handshakes from the client namespace. These readiness handshakes
contain no HTTP request and therefore cannot invoke model generation.
