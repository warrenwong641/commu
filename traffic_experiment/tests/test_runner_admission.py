...
from traffic_experiment.traffic_measure.runner import RunSettings, run_experiment
...
def _run_with_clock(
    ...
    *admission_elapsed* lists the session-elapsed seconds at which
    ...
    with (
        mock.patch(
            "traffic_experiment.traffic_measure.runner.httpx.Client",
            new=_FakeClient,
        ),
        mock.patch(
            "traffic_experiment.traffic_measure.runner.time.perf_counter",
            new=fake_perf_counter,
        ),
    ):
