...
test_caddyfile_syntax() {
  ...
  local caddy
  caddy="$(find "${PROJECT_DIR}/.tools" -name caddy -type f 2>/dev/null | head -1 || true)"
...
test_profile_separation() {
  ...
  std_out="$(grep -E '^(OUTPUT_DIR|ATTEMPT_ROOT)=' "${script}" | head -1 || true)"
...
test_owned_lifecycle_guards() {
  local proxy="${PROJECT_DIR}/scripts/07_start_secure_proxy.sh"
  local preliminary="${PROJECT_DIR}/scripts/12_run_preliminary_resumable.sh"
  local matrix="${PROJECT_DIR}/scripts/18_run_lab_matrix.sh"
  local sessions="${PROJECT_DIR}/scripts/19_run_lab_sessions.sh"
  local validation="${PROJECT_DIR}/scripts/22_validate_protocol_pilots.sh"
  local listener="${PROJECT_DIR}/scripts/23_server_physical_listener.sh"
  local unsafe

  unsafe="$(grep -En '^[[:space:]]*(pkill|caddy[[:space:]]+stop)([[:space:]]|$)' \
    "${proxy}" "${preliminary}" "${matrix}" "${sessions}" "${validation}" "${listener}" || true)"
  if [[ -z "${unsafe}" ]]; then
    pass "lifecycle: no broad pkill or shared-admin caddy stop"
  else
    fail "lifecycle: unsafe process stop remains: ${unsafe}"
  fi

  if grep -Fq 'trap cleanup EXIT' "${preliminary}" &&
    grep -Fq 'trap cleanup EXIT' "${validation}"; then
    pass "lifecycle: namespace pilots install EXIT cleanup"
  else
    fail "lifecycle: missing namespace-pilot EXIT cleanup"
  fi

  if grep -Fq 'NETWORK_OWNED=1' "${preliminary}" &&
    grep -Fq 'NETWORK_OWNED=1' "${validation}" &&
    grep -Fq 'Refusing to replace existing' "${preliminary}" &&
    grep -Fq 'Refusing to replace existing' "${validation}"; then
    pass "lifecycle: namespace cleanup is scoped to initially absent resources"
  else
    fail "lifecycle: namespace ownership guards are incomplete"
  fi

  if grep -Fq 'caddy_start_ticks=' "${listener}" &&
    grep -Fq 'caddy_config=' "${listener}" &&
    grep -Fq '_caddy_pid_matches' "${listener}" &&
    grep -Fq '_stop_verified_caddy' "${listener}"; then
    pass "lifecycle: physical listener records and verifies PID identity"
  else
    fail "lifecycle: physical listener PID reuse guard is incomplete"
  fi

  if grep -Fq 'caddy_start_ticks=' "${proxy}" &&
    grep -Fq 'caddy_config=' "${proxy}" &&
    grep -Fq 'caddy_pid_matches' "${proxy}" &&
    grep -Fq 'stop_recorded_caddy' "${proxy}"; then
    pass "lifecycle: secure proxy records and verifies exact process identity"
  else
    fail "lifecycle: secure proxy ownership state is incomplete"
  fi

  if grep -Fq 'trap cleanup_on_exit EXIT' "${matrix}" &&
    grep -Fq 'trap cleanup_on_exit EXIT' "${sessions}" &&
    grep -Fq 'CADDY_STATE_FILE=' "${matrix}" &&
    grep -Fq 'CADDY_STATE_FILE=' "${sessions}"; then
    pass "lifecycle: lab orchestrators use scoped Caddy state and EXIT cleanup"
  else
    fail "lifecycle: lab orchestrator Caddy cleanup is incomplete"
  fi

  if grep -Fq 'NETWORK_OWNED=1' "${matrix}" &&
    grep -Fq 'NETWORK_OWNED=1' "${sessions}" &&
    grep -Fq 'Refusing to replace existing' "${matrix}" &&
    grep -Fq 'Refusing to replace existing' "${sessions}"; then
    pass "lifecycle: lab orchestrators refuse pre-existing namespace/veth names"
  else
    fail "lifecycle: lab orchestrator namespace ownership guards are incomplete"
  fi

  if grep -Fq '[[ "${NETWORK_OWNED}" -eq 1 ]] || return 0' "${matrix}" &&
    grep -Fq '[[ "${NETWORK_OWNED}" -eq 1 ]] || return 0' "${sessions}" &&
    [[ "$(grep -Fc '"${SCRIPT_DIR}/11_network_condition.sh" reset' "${matrix}")" -eq 1 ]] &&
    [[ "$(grep -Fc '"${SCRIPT_DIR}/11_network_condition.sh" reset' "${sessions}")" -eq 1 ]]; then
    pass "lifecycle: lab namespace reset exists only in owned-resource cleanup"
  else
    fail "lifecycle: lab namespace reset is not strictly ownership-gated"
  fi
}
