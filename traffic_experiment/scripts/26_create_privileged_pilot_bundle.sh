...
# Build a credential-free, checksum-verifiable release for the two protocol
# pilots. Run this as the service user. Root only consumes the finished archive
# through 27_install_privileged_pilot_release.sh.
...
BUILD_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/commu-pilot-release.XXXXXX")"
cleanup() {
  local status=$?
  trap - EXIT
  [[ "${BUILD_ROOT}" == "${TMPDIR:-/tmp}"/commu-pilot-release.* ]] &&
    rm -rf -- "${BUILD_ROOT}"
  exit "${status}"
}
trap cleanup EXIT
...
"${PYTHON}" "${SCRIPT_DIR}/privileged_pilot_config.py" normalize \
  --input "${CONFIG}" \
  --output "${RELEASE_ROOT}/config/server.env" \
  --repository-sha "${REPOSITORY_SHA}"

QA_RELATIVE="$(
  "${PYTHON}" "${SCRIPT_DIR}/privileged_pilot_config.py" get \
...
# distribution constrained by the committed pilot-only lock hashes.
...
repository_sha=${REPOSITORY_SHA}
purpose=protocol-pilots-only
EOF

"${PYTHON}" "${SCRIPT_DIR}/privileged_pilot_config.py" check \
  --input "${RELEASE_ROOT}/config/server.env" \
  --repository-sha "${REPOSITORY_SHA}" \
...
printf 'PRIVILEGED_PILOT_BUNDLE_OK repository_sha=%s\n' "${REPOSITORY_SHA}"
printf 'bundle=%s\n' "${OUTPUT}"
printf 'bundle_sha256=%s\n' "$(sha256sum -- "${OUTPUT}" | awk '{print $1}')"
printf 'contains_credentials=false\n'
printf 'purpose=protocol-pilots-only\n'
