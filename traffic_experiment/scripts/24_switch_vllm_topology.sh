...
prepare_new_log() { local parent; parent="$(dirname -- "$1")"; [[ "$1" = /* && -d "${parent}" && ! -L "${parent}" && "$(stat -c %u -- "${parent}")" == "$(id -u)" && ! -e "$1" && ! -L "$1" ]] || return 1; (set -o noclobber; : >"$1") 2>/dev/null || return 1; [[ "$(stat -c %F -- "$1")" == "regular file" && "$(stat -c %u -- "$1")" == "$(id -u)" && "$(stat -c %h -- "$1")" == 1 ]] || return 1; chmod 600 "$1"; }
