...
if [[ "${COMMU_PRIVILEGED_MATRIX_INSTALL_CLEAN_ENV:-}" != 1 ]]; then
  ...
  exec /usr/bin/env -i \
    COMMU_PRIVILEGED_MATRIX_INSTALL_CLEAN_ENV=1 \
...
while IFS='=' read -r inherited_name _; do
  case "${inherited_name}" in
    COMMU_PRIVILEGED_MATRIX_INSTALL_CLEAN_ENV|HOME|LANG|LC_ALL|TZ|PATH|PYTHONNOUSERSITE|PYTHONDONTWRITEBYTECODE|PYTHONSAFEPATH|PWD|SHLVL|_) ;;
...
BASE=/opt/commu-secure-matrix/releases
OUTPUT_BASE=/var/lib/commu-secure-matrix
...
EXPECTED_REVIEWED_CODE_MANIFEST_SHA256=e1bee88d9dd931ff2ada54d037c8d5d33c4406ec78dd54df019d0f11a900a20b
ARCHIVE="${1:-}"
...
for executable in \
  ...
  /usr/bin/dirname /usr/bin/id /usr/bin/install /usr/bin/kill /usr/bin/ln /usr/bin/mktemp /usr/bin/mv \
...
if sys.implementation.name != "cpython" or sys.version_info[:2] != (3, 12):
    raise SystemExit("ERROR: privileged matrix release requires system CPython 3.12")
if platform.machine() != "x86_64" or libc_name != "glibc":
    raise SystemExit("ERROR: privileged matrix release requires x86_64 Linux with glibc")
if tuple(map(int, libc_version.split(".")[:2])) < (2, 28):
    raise SystemExit("ERROR: privileged matrix wheels require glibc >= 2.28")
PY

/usr/bin/install -d -o root -g root -m 0755 /opt/commu-secure-matrix "${BASE}"
...
VALIDATOR="${VALIDATED_CODE_ROOT}/repository/traffic_experiment/scripts/privileged_matrix_config.py"
early_value() {
...
[[ "$(early_value schema "${EARLY_METADATA}")" == commu-privileged-matrix-release-v1 &&
...
[[ "$(early_config_value VLLM_MODEL)" == "${EXPECTED_MODEL}" &&
  ...
  die "bundle config is outside this installer's authorized matrix scope"

/usr/bin/install -d -o root -g "${EXPECTED_SERVICE_GID}" -m 0710 "${OUTPUT_BASE}"
...
[[ "$(metadata_value purpose)" == secure-single-gpu-full-matrix ]] || die "release is not the fixed full matrix"
...
[[ "$(config_value VLLM_MODEL)" == "${EXPECTED_MODEL}" &&
  ...
  die "bundle config is outside the independently authorized matrix scope"
...
# committed minimal runtime lock while --no-index prevents network substitution.
...
# The topology switcher, pilot runner, and matrix supervisor flock this inode.
...
[[ -f "${GLOBAL_LOCK}" && ! -L "${GLOBAL_LOCK}" &&
  ...
  die "pilot-installed shared topology lock is absent or unsafe"
