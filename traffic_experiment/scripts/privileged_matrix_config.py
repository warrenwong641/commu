#!/usr/bin/env python3
"""Validate inert configuration used by the root-owned full-matrix release."""
...
def validate_release_config(
    ...
    for key, expected in exact.items():
        if key not in values or values[key] != expected:
            raise ConfigError(f"{key} must equal {expected!r} for the privileged matrix")
    ...
    expected_calls = (
        3
        * 2
        * int(require(values, "LAB_REPETITIONS"))
        * (
            int(require(values, "LAB_QA_SAMPLES")) * 3
            + int(require(values, "LAB_SUMMARY_SAMPLES")) * 3
        )
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"privileged matrix configuration rejected: {exc}", file=sys.stderr)
        return 2
