...
The config may use @REPOSITORY_SHA@, @GPU_INDEX@, and @GPU_SCOPE@ placeholders.
The bundle contains no API key and can launch only the fixed one-GPU
2,808-call matrix for a service state selected at runtime.
EOF
}
...
SERVICE_STATE_ROOT="$(canonical_directory "${SERVICE_STATE_ROOT}")" ||
...
schema=commu-privileged-matrix-policy-v2
